...
export default function LanguageHomePage({ params }: { params: { lang: LanguageCode } }) {
  ...
  const pageContent = pageContentMap[params.lang] || pageContentMap.en;
                     ...
                     <div className="md:w-2/5 xl:w-1/3">
                        {post.image && (
                          <div className="relative w-full h-full min-h-[200px] md:min-h-0 aspect-video md:aspect-[1.91/1] overflow-hidden">
                            <Image
                              src={post.image}
                              alt={post.content[params.lang]?.title || post.content['en'].title}
                              fill
                      ...
                      <div className="md:w-3/5 xl:w-2/3 flex flex-col justify-center p-6 lg:p-8">
                        <h3 className="text-xl lg:text-2xl font-bold mb-2 text-foreground">{post.content[params.lang]?.title || post.content['en'].title}</h3>
                        <p className="text-sm text-muted-foreground mb-4">
                          {pageContent.postedOn} {format(new Date(post.date), 'MMMM d, yyyy')}
                        </p>
                        <p className="text-base text-muted-foreground line-clamp-2">{post.content[params.lang]?.description || post.content['en'].description}</p>
                      </div>
                   </div>
                </Card>
              </Link>
